...
pg_tde_set_db_file_paths(const RelFileLocator *rlocator, char *str_append)
{
	...
	db_map_path = psprintf("%s/%s%s", db_path, PG_TDE_MAP_FILENAME, ((str_append) ? str_append : ""));
	db_keydata_path = psprintf("%s/%s%s", db_path, PG_TDE_KEYDATA_FILENAME, ((str_append) ? str_append : ""));
}
...
{
	db_path = db_map_path = db_keydata_path = NULL;
}
...
/*
 * Base on the given arguments, creates and write the entry into the key
 * map file.
 */
int32
...
pg_tde_write_one_keydata(File keydata_file, off_t header_size, int32 key_index, RelKeysData *enc_rel_key_data)
{
	...
	elog(INFO, "keysize = %zu => curr_pos = %zu", key_size, curr_pos);
...
pg_tde_read_one_keydata(File keydata_file, off_t header_size, int32 key_index, const char *master_key_name)
{
	...
	read_pos += (key_index * SizeOfRelKeysData(key_count));
	...
	if (FileRead(keydata_file, enc_rel_key_data->internal_key, key_data_size, read_pos, WAIT_EVENT_DATA_FILE_READ) != key_data_size)
	{
		ereport(FATAL,
				(errcode_for_file_access(),
...
{
	/*
	 * Discussion Pending Around Master Key Rotation
	 * - Currently, the code has all the required cogs in places
	 * 		-> All expected helper functions are created and test.
	 * 
	 * Implementation:
